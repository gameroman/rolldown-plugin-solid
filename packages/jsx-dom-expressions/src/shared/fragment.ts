...
import type { TransformContext, TransformResult, PluginConfig, Node, JSXText } from "../types";
...
export default function transformFragmentChildren(
  ...
  const childNodes = filteredChildren.reduce((memo: any[], child) => {
    if (is.JSXText(child)) {
      ...
      const childResult = transformNode(ctx, child, { topLevel: true, fragmentChild: true, lastElement: true });
      if (childResult) {
  ...
  results.exprs.push(
    b.ExpressionStatement({
      expression: childNodes.length === 1 ? childNodes[0] : b.ArrayExpression({ elements: childNodes })
    })
  );
}
