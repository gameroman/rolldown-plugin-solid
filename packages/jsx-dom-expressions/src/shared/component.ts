...
import type { TransformContext, TransformResult, Expression, Identifier, JSXElement, Node } from "../types";
import {
  getConfig,
  isDynamic,
...
function convertComponentIdentifier(node: any): Expression {
  if (is.JSXIdentifier(node)) {
    ...
    if (/^[a-zA-Z_$][a-zA-Z0-9_$]*$/.test(node.name)) return b.Identifier({ name: node.name });
...
export default function transformComponent(ctx: TransformContext, node: JSXElement): TransformResult {
  ...
  if (is.Identifier(tagId) && config.builtIns.indexOf((tagId as Identifier).name) > -1) {
  ...
  node.openingElement.attributes.forEach((attr: any) => {
    if (is.JSXSpreadAttribute(attr)) {
      ...
      props.push(
        isDynamic(ctx, attr.argument, { checkMember: true }) && (dynamicSpread = true)
      ...
      const value =
          (is.StringLiteral(attr.value) ? b.Literal({ value: (attr.value ).value }) : attr.value) ||
      ...
      if (is.JSXExpressionContainer(value)) {
        if (key === "ref") {
          ...
          if (!isConstant && is.Identifier(valueExpr)) {
            ...
            runningObject.push(b.Property({ key: b.Identifier({ name: "ref" }), value: valueExpr, computed: false, shorthand: false }));
          }
        } else if (
            ...
            runningObject.push(
              b.Property({
                ...
                value: b.Function({
                  ...
                  id: null,
                  params: [],
                  body: b.BlockStatement({ body: [b.ReturnStatement({ argument: expr.body || expr })] }),
                  generator: false,
                  async: false,
                  declare: false,
                  typeParameters: null,
            ...
            runningObject.push(
              b.Property({
                ...
                value: b.Function({
                  ...
                  id: null,
                  params: [],
                  body: b.BlockStatement({ body: [b.ReturnStatement({ argument: (value ).expression })] }),
                  generator: false,
                  async: false,
                  declare: false,
                  typeParameters: null,
          ...
          runningObject.push(b.Property({ key: id, value: (value ).expression, computed: false, shorthand: false }));
        }
      } else {
        runningObject.push(b.Property({ key: id, value, computed: false, shorthand: false }));
      }
    }
  });
  ...
  if (childResult && childResult[0]) {
    if (childResult[1]) {
      runningObject.push(
        b.Property({
          ...
          value: b.Function({
            ...
            id: null,
            params: [],
            body: b.BlockStatement({ body: [b.ReturnStatement({ argument: childResult[0] })] }),
            generator: false,
            async: false,
            declare: false,
            typeParameters: null,
      ...
      runningObject.push(b.Property({ key: b.Identifier({ name: "children" }), value: childResult[0], computed: false, shorthand: false }));
    }
  }

  if (runningObject.length || !props.length) props.push(b.ObjectExpression({ properties: runningObject }));
  if (props.length > 1 || dynamicSpread) {
    props = [b.CallExpression({ callee: registerImportMethod(ctx, "mergeProps"), arguments: props, optional: false })];
  }
  ...
  return { exprs, template: "", component: true, dynamics: [], postExprs: [], declarations: [] };
}

function transformComponentChildren(ctx: TransformContext, children: Node[], config: TransformContext["config"]): [Expression, boolean] | null {
  const filteredChildren = filterChildren(children);
  if (!filteredChildren.length) return null;
  let dynamic = false;

  let transformedChildren: any[] = filteredChildren.reduce((memo: any[], child) => {
    if (is.JSXText(child)) {
  ...
  if (transformedChildren.length === 1) {
    ...
    if (
      ...
      result = is.CallExpression(result) && !(result ).arguments.length && !is.Identifier(result.callee)
        ? result.callee
