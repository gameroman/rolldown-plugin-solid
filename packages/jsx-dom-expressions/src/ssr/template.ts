...
import type { TransformContext, TransformResult, Expression, Statement, VariableDeclarator } from "../types";
...
export function createTemplate(ctx: TransformContext, result: TransformResult): Expression {
  if (!result.template) {
    return result.exprs[0] ? getExpression(result.exprs[0]) : b.Identifier({ name: "undefined" });
  }
  ...
  const found = ctx.templates.find(tmp => tmp.template === getTemplateString(result.template));
  ...
  const args: Expression[] = Array.isArray(result.template) && result.template.length > 1
