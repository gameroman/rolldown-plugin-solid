  ...
  for (const [name, value] of privateNamesMap) {
    ...
    if (privateFieldsAsProperties) {
      ...
      init = t.newExpression(
        t.identifier(
          isMethod && (undefined || !isGetterOrSetter || newHelpers(state))
            ? "WeakSet"
            : "WeakMap",
        ),
        [],
...
function writeOnlyError(file: File, name: string) {
  if (!undefined && !file.availableHelper("writeOnlyError")) {
    console.warn(
