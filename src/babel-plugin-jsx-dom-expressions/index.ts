...
function isDynamic(
  ...
  if (
    checkTags &&
    (t__namespace.isJSXElement(expr) ||
      (t__namespace.isJSXFragment(expr) && expr.children.length))
  ) {
    return true;
  }
...
function transformElement$3(path, info) {
  ...
  if (info.topLevel && config.hydratable && results.hasHydratableEvent) {
    ...
    results.postExprs.push(
      t__namespace.expressionStatement(
        t__namespace.callExpression(runHydrationEvents, []),
      ),
    );
  }
  if (wrapSVG) {
...
function setAttr$2(
  ...
  if (namespace === "style") {
    ...
    return t__namespace.callExpression(setStyleProperty, [
      ...
      t__namespace.isAssignmentExpression(value) &&
      t__namespace.isIdentifier(value.left)
        ? value.right
        : value,
    ]);
  }
  ...
  if (namespace === "class") {
    return t__namespace.callExpression(
      t__namespace.memberExpression(
        t__namespace.memberExpression(
          elem,
          t__namespace.identifier("classList"),
        ),
      ...
      [
        ...
        dynamic
          ...
          : t__namespace.unaryExpression(
              "!",
              t__namespace.unaryExpression("!", value),
            ),
      ],
    );
  }
  ...
  if (dynamic && name === "textContent") {
    if (config.hydratable) {
      return t__namespace.callExpression(
        registerImportMethod(path, "setProperty"),
        [elem, t__namespace.stringLiteral("data"), value],
      );
    }
  ...
  if (
    ...
    if (config.hydratable && namespace !== "prop") {
      return t__namespace.callExpression(
        registerImportMethod(path, "setProperty"),
        [elem, t__namespace.stringLiteral(alias || name), value],
      );
    }
    return t__namespace.assignmentExpression(
      "=",
      t__namespace.memberExpression(
        elem,
        t__namespace.identifier(alias || name),
      ),
      value,
    );
  }
...
function transformAttributes$2(path, results) {
  ...
  const tagName = getTagName(path.node),
    ...
    isCE =
      tagName.includes("-") ||
      attributes.some((a) => a.node.name?.name === "is"),
  ...
  if (
    attributes.some((attribute) =>
      t__namespace.isJSXSpreadAttribute(attribute.node),
    )
  ) {
        ...
        for (let i = 0; i < properties.length; i++) {
          ...
          if (t__namespace.isObjectProperty(property)) {
            ...
            if (
              ...
              (t__namespace.isIdentifier(property.value) &&
  ...
  const styleAttribute = path
    ...
    .find(
      (a) =>
        ...
        !a.node.value.expression.properties.some((p) =>
      ...
      if (
        ...
        if (
          ...
          if (
            ...
            value = t__namespace.jsxExpressionContainer(
              t__namespace.numericLiteral(evaluated),
            );
      ...
      if (
                ...
                results.exprs.unshift(
                  t__namespace.expressionStatement(
                    t__namespace.assignmentExpression(
                      ...
                      t__namespace.memberExpression(
                        elem,
                        t__namespace.identifier(`$$${ev}Data`),
                      ),
                      handler.elements[1],
                    ),
                  ),
         ...
         */
          ...
          if (
            ...
            results.exprs.push(
              t__namespace.expressionStatement(
                setAttr$2(attribute, elem, key, value, {
                  isSVG,
                  isCE,
                  tagName,
                }),
          ...
          if (t__namespace.isJSXExpressionContainer(content))
...
function nextChild$1(children, index) {
  return (
    children[index + 1] &&
    (children[index + 1].id || nextChild$1(children, index + 1))
  );
}
  ...
  results.exprs.push(
    t__namespace.expressionStatement(
      t__namespace.assignmentExpression(
        ...
        t__namespace.memberExpression(
          results.id,
          t__namespace.identifier("_$owner"),
        ),
        t__namespace.callExpression(
          registerImportMethod(
            path,
...
function processSpreads$1(
  ...
  attributes.forEach((attribute) => {
    ...
    if (t__namespace.isJSXSpreadAttribute(node)) {
      ...
      spreadArgs.push(
        isStatic
          ? t__namespace.objectExpression([t__namespace.spreadElement(s)])
          : s,
      );
    } else if (
      (firstSpread ||
...
function createTemplate$1(path, result) {
  ...
  if (!Array.isArray(result.template)) {
    ...
    const strings = result.template.map((tmpl) =>
      t__namespace.stringLiteral(tmpl),
    );
  ...
  const found = templates.find((tmp) => {
    if (
      t__namespace.isArrayExpression(tmp.template) &&
...
function transformElement$2(path, info) {
  ...
  if (
    path.node.openingElement.attributes.some((a) =>
      t__namespace.isJSXSpreadAttribute(a),
    )
  )
  ...
  if (info.topLevel && config.hydratable) {
    if (tagName === "head") {
      ...
      results.exprs.push(
        t__namespace.callExpression(
          t__namespace.identifier("_$createComponent"),
          [
...
function escapeExpression(path, expression, attr, escapeLiterals) {
  if (
    ...
    (t__namespace.isTemplateLiteral(expression) &&
    ...
    t__namespace.isJSXElement(expression) &&
...
function transformToObject(attrName, attributes, selectedAttributes) {
  ...
  for (let i = 0; i < selectedAttributes.length; i++) {
    ...
    properties.push(
      t__namespace.objectProperty(
        computed
          ? t__namespace.stringLiteral(attr.name.name.name)
          : attr.name.name,
        t__namespace.isJSXExpressionContainer(attr.value)
          ? attr.value.expression
          : attr.value,
...
function normalizeAttributes(path) {
  const attributes = path.get("openingElement").get("attributes"),
    styleAttributes = attributes.filter(
      (a) =>
        t__namespace.isJSXNamespacedName(a.node.name) &&
    ...
    for (let i = 0; i < classAttributes.length; i++) {
      ...
      if (!t__namespace.isJSXExpressionContainer(attr.value)) {
        ...
        if (attr.name.name === "classList") {
          ...
          expr = t__namespace.callExpression(
      ...
      if (
        ...
        if (key === "style") {
          if (
            ...
            !value.expression.properties.some((p) =>
              t__namespace.isSpreadElement(p),
            )
          ) {
            const props = value.expression.properties.map((p, i) =>
              t__namespace.callExpression(
                ...
                [
                  t__namespace.stringLiteral(
                    (i ? ";" : "") +
                      (t__namespace.isIdentifier(p.key)
                        ? p.key.name
                        : p.key.value) +
                      ":",
                  ),
        ...
        if (key === "classList") {
          if (
            ...
            if (!values.length)
              value.expression = t__namespace.stringLiteral(
                quasis[0].value.raw,
              );
            else if (
              values.length === 1 &&
...
function transformChildren$1(path, results, { hydratable }) {
  ...
  filteredChildren.forEach((node) => {
    if (
      t__namespace.isJSXElement(node.node) &&
      getTagName(node.node) === "head"
    ) {
      ...
      if (t__namespace.isJSXSpreadAttribute(node)) {
        ...
        if (t__namespace.isJSXExpressionContainer(value)) {
          if (
            ...
            runningObject.push(
              t__namespace.objectProperty(id, value.expression),
            );
        ...
        if (t__namespace.isJSXExpressionContainer(value)) {
          if (key === "ref") {
            ...
            if (!isConstant && t__namespace.isLVal(value.expression)) {
                  ...
                  t__namespace.blockStatement([
                    ...
                    t__namespace.expressionStatement(
                      t__namespace.conditionalExpression(
                        ...
                        t__namespace.callExpression(refIdentifier, [
                          t__namespace.identifier("r$"),
                        ]),
                        t__namespace.assignmentExpression(
                          "=",
              ...
              !isConstant &&
              t__namespace.isOptionalMemberExpression(value.expression)
            ) {
              const refIdentifier = path.scope.generateUidIdentifier("_ref$");
              runningObject.push(
                t__namespace.objectMethod(
        ...
        if (parent) {
          ...
          return t__namespace.callExpression(
            t__namespace.arrowFunctionExpression(
              ...
              t__namespace.blockStatement([
                decl,
                t__namespace.returnStatement(node),
              ]),
            ),
            [],
          );
